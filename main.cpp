...
/*int main() {
    ...
    const int STEPS = 64; // how many frames (files)
    const int TILE = 16;
    ...
    int mid = M / 2;
    A[idx(mid, mid, mid, M)] = 1000000000000.0;
    ...
    //TODO: learn bitwise bs so that we can apply morton ordering
    ...
    for (int t = 1; t <= STEPS; ++t) {
        ...
        for (const auto& tile : tiles) {
            const int bk = tile.bk, bi = tile.bi, bj = tile.bj;

            const int baseK = bk * TILE;
            const int baseJ = bj * TILE;
            const int baseI = bi * TILE;

            //now we switch to row major for the inside of the tiles, basically back to normal now
...
int main() {
    ...
    //TODO: learn bitwise stuff so that we can apply morton ordering
